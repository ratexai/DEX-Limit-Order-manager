...
// RateLimitedClient wraps a ChainClient with a channel-based token-bucket
// rate limiter to prevent RPC throttling. The host configures the rate when
// constructing the ChainInstance.
//
// Usage:
//
//	client := positionmanager.NewRateLimitedClient(ethclient, 50) // 50 RPC/sec
//	cfg.Chains[1] = ChainInstance{Client: client, ...}
//	defer client.Close() // stops the refill goroutine
